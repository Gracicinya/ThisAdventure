/* ========================================
   ...
   Handles three things on the Characters page:
     1. The character slider (rotating featured cards)
     2. The character grid with filter buttons
     3. The character modal (full story popup)
======================================== */


/* ========================================
   CHARACTER DATA
   Each object holds all the information for one character.
   This data is used to build the slider, grid, and modal.
======================================== */
var characters = [
  {
    id: 0,
...
/* ========================================
   PORTRAIT HELPER
   Builds the HTML for a character portrait image.
   If the image fails to load, a fallback icon is shown instead.

   Parameters:
     ch      — the character object
     classes — optional extra CSS classes for the image
======================================== */
function buildPortrait(ch, classes) {
  /* Use an empty string if no extra classes are given */
  var extraClasses = classes ? ' ' + classes : '';

  return '<img'
    + ' class="portrait-img' + extraClasses + '"'
    + ' src="' + ch.image + '"'
...
/* ========================================
   BUILD THE SLIDER
   Creates a sliding card for each character and adds navigation dots.
======================================== */

/* Get the slider track and dots container from the page */
...
/* Loop through every character and build their slider card */
characters.forEach(function(ch, i) {

  /* Get the stat names (strength, magic, resolve) */
  var statKeys = Object.keys(ch.stats);

  /* Build the stat bars HTML */
  var statBarsHTML = '';
  statKeys.forEach(function(k) {

    /* Work out which colour the bar should be */
    var fillClass = '';
    if (ch.role === 'villain') {
      fillClass = 'enemy-fill';
    } else if (ch.role === 'protagonist' && ch.magicType === 'dark') {
      fillClass = 'dark-fill';
    }

    /* Capitalise the first letter of the stat name */
    var label = k.charAt(0).toUpperCase() + k.slice(1);

    statBarsHTML += '<div class="stat">'
      + '<span class="stat-label">' + label + '</span>'
      + '<div class="stat-bar">'
      + '<div class="stat-fill ' + fillClass + '" style="width:' + ch.stats[k] + '%"></div>'
      + '</div>'
      + '</div>';
  });
  ...
  card.innerHTML = '<div class="slider-img-wrap">'
    ...
    + '<div class="char-stats">' + statBarsHTML + '</div>'
    + '<button class="learn-more-btn" onclick="openModal(' + i + ')">Read full story &#8599;</button>'
    + '</div>';

  /* Add this card to the slider */
  if (sliderTrack) sliderTrack.appendChild(card);

  /* Create a dot for this slide */
  var dot = document.createElement('div');
  dot.className = 'dot' + (i === 0 ? ' active' : ''); /* first dot starts active */
  dot.addEventListener('click', function() { goToSlide(i); });
  if (sliderDots) sliderDots.appendChild(dot);
});


/* Move the slider to a specific slide number */
function goToSlide(n) {
  currentSlide = n;
  if (!sliderTrack) return;

  /* Shift the track left by 100% for each slide */
...
/* Go to the previous slide (wraps around to the last one) */
...
/* Go to the next slide (wraps around to the first one) */
...
/* ========================================
   ...
   Restarted whenever the user interacts with the slider.
======================================== */
...
/* Only start the timer if the slider exists on this page */
if (sliderTrack) restartSliderInterval();


/* ========================================
   SWIPE SUPPORT
   Lets users swipe left/right on touch screens to change slides.
======================================== */
if (sliderTrack) {
  ...
  var SWIPE_THRESHOLD = 50; /* minimum px moved to count as a swipe */
  ...
  sliderTrack.addEventListener('touchstart', function(e) {
    ...
    clearInterval(sliderInterval); /* pause auto-advance while swiping */
  }, { passive: true });
  ...
  /* When the finger lifts, decide which direction was swiped */
  sliderTrack.addEventListener('touchend', function() {
    ...
    isTouching = false;
    restartSliderInterval(); /* resume auto-advance */
  }, { passive: true });

  /* Also support pointer events (some devices use these instead of touch events) */
...
/* ========================================
   ...
   Creates a small card for each character in the grid section.
   Clicking a card opens the full modal.
======================================== */
...
if (charGrid) {
  characters.forEach(function(ch, i) {
    ...
    card.dataset.role = ch.role; /* used by the filter buttons */

    card.innerHTML = '<div class="card-portrait" style="background:' + ch.bgColor + ';">'
...
/* ========================================
   FILTER BUTTONS
   Clicking a filter button shows only the cards matching that role.
...
document.querySelectorAll('.filter-btn').forEach(function(btn) {
  ...
  btn.addEventListener('click', function() {
    ...
    /* Show or hide each card depending on the selected filter */
    document.querySelectorAll('.char-card').forEach(function(card) {
      var shouldShow = (filter === 'all') || (card.dataset.role === filter);
...
/* ========================================
   MODAL
   Opens a popup with the character's full story.
   Closes when the X button, the background, or Escape is pressed.
======================================== */
var overlay      = document.getElementById('modalOverlay');
var modalContent = document.getElementById('modalContent');

/* Opens the modal for the character at the given index */
function openModal(index) {
  ...
  /* Build the lore paragraphs HTML */
  var loreHTML = '';
  ch.lore.forEach(function(p) {
    loreHTML += '<p>' + p + '</p>';
  });

  /* Fill the modal with the character's information */
  modalContent.innerHTML = '<div class="modal-portrait-wrap" style="background:' + ch.bgColor + ';">'
    + buildPortrait(ch, 'modal-portrait-img')
    + '<div class="modal-portrait-icon portrait-icon-fallback" style="display:none;"></div>'
    + '</div>'
    ...
    + '<div class="char-stats" style="margin-bottom:1.8rem;">' + statBarsHTML + '</div>'
    + '<div class="modal-lore">' + loreHTML + '</div>'
    + '<div class="modal-quote">' + ch.quote + '</div>';

  /* Show the modal and stop the page from scrolling behind it */
  overlay.classList.add('open');
  overlay.removeAttribute('aria-hidden');
  document.body.style.overflow = 'hidden';
}

/* Closes the modal and allows the page to scroll again */
function closeModal() {
...
/* Close when the user clicks on the dark background (not the modal box itself) */
if (overlay) {
